                ...
                <li key={menuItem.id} className={menuItem.subMenu.length > 0 ? 'dropdown' : ''}>
                    <Link href={
                        ...
                        menuItem.menuItem === "Blog" ? "/news-grid" :
                         `/page-${menuItem.menuItem.toLowerCase().replace(/\s+/g, '-')}`}>
                        {menuItem.menuItem}
                    </Link>
                    ...
                    {menuItem.subMenu.length > 0 && (
                        <ul>
                            {menuItem.subMenu.map((subMenuItem) => (
                                ...
                                >
                                    {/* <Link href={`/service-${subMenuItem.subServices.toLowerCase().replace(/\s+/g, '-')}`}>
                                    ...
                                    <Link href={
                                        ...
                                        ? "/service-details"
                                        : `/service-${subMenuItem.subServices.toLowerCase().replace(/\s+/g, '-').replace(/[()]/g, '')}`
                                    }>
                                        {subMenuItem.subServices}
                                    </Link>

                                    {/* {dropdowns[subMenuItem.id] && subMenuItem.subChild && (
                                        <ul className="sub-dropdown">
                                            {subMenuItem.subChild.map((childItem) => (
                                                <li key={childItem.id}>
                                                    <Link href={`/service-${childItem.childList.toLowerCase().replace(/\s+/g, '-')}`}>
                                                        {childItem.childList}
                                                    </Link>
                                                </li>
                                            ))}
